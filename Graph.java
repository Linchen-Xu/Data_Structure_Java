public class Graph<T>{
	...
	public int getWeight(int v1,int v2){
		...
		if ((v1>=0 && v1<verticesList.size()) &&(v2>=0 && v2<verticesList.size()))
			return edges[v1][v2];
		else
	...
	public int getNextNeighbor(int v1,int v2){
		...
		int col;
		if((v1>=0 && v1<verticesList.size())&&(v2>=0 && v2<verticesList.size()-1))
			for(col = v2+1;col < verticesList.size();col++)
				if(edges[v1][col] > 0&&edges[v1][col] < Integer.MAX_VALUE)
					return col;
		return -1;
	}
